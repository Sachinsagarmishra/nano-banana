import { getUser, getSupabaseAdmin, setCors } from '../lib/supabase.js';
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Check auth (optional — allow unauthenticated for now, but log if authed)
        const user = await getUser(req);
        let userId = user?.id || null;

        // Get API key from Supabase settings (falls back to env var)
        ...
        // Log generation and deduct credits
        if (data.code === 200 && userId) {
            ...
            // Deduct credit
            await supabase.rpc('', {}).catch(() => { }); // Fallback to manual
            await supabase
                .from('profiles')
                .update({
